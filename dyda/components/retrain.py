...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
  ...
  for sub_dir in sub_dirs:
    if is_root_dir:
      is_root_dir = False
      continue
    extensions = ['jpg', 'jpeg', 'JPG', 'JPEG', 'jpg.0']
    file_list = []
...
def main(_):
  ...
  with tf.Session(graph=graph) as sess:
    ...
    for i in range(FLAGS.how_many_training_steps):
      ...
      if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
        train_accuracy, cross_entropy_value = sess.run(
            ...
            feed_dict={bottleneck_input: train_bottlenecks,
                       ground_truth_input: train_ground_truth})
        result_temp["training_step"] = i
        tf.logging.info('%s: Step %d: Train accuracy = %.1f%%' %
                        (datetime.now(), i, train_accuracy * 100))
        result_temp["train_accu"] = train_accuracy
        tf.logging.info('%s: Step %d: Cross entropy = %f' %
                        (datetime.now(), i, cross_entropy_value))
        result_temp["cross_entropy"] = cross_entropy_value
        ...
        result_temp["val_accu"] = validation_accuracy
        results["training"].append(copy.deepcopy(result_temp))
    ...
    results["testing"] = {"test_accu": test_accuracy}
    tools.write_json(results, fname="output.json")
